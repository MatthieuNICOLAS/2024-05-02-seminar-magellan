...
\begin{frame}{Operation-based synchronisation}
    \begin{itemize}
        ...
        \item An operation \alert{correponds to one or several \emph{irreducible elements}}
    \end{itemize}
        ...
        \resizebox{\columnwidth}{!}{
          \begin{tikzpicture}
            \path
                ...
                +(0:1) node[point, label=above right:{$\set{a,e}$}] (a-initial) {}
                +(0:3.5) node[point, label=above right:{$\set{a,b,e}$}, label=-170:{$\trm{add}(b)$}] (a-add-b) {}
                +(0:7) node[point, label=above right:{$\set{a,b,c,e}$}, label=-170:{$\trm{add}(c)$}] (a-add-c) {}
                +(0:12.5) node[point, label=above right:{$\set{a,b,c,d,e}$}] (a-receives-add-d) {};

            \draw[dotted] (a) -- (a-initial) (a-receives-add-d) -- (a-end);
            \draw[->, thick] (a-initial) --  (a-add-b) -- (a-add-c) -- (a-receives-add-d);
            ...
            \path
                ...
                +(0:9) node[point, label=below right:{$\set{a,b,d,e}$}] (b-receives-add-b) {}
                +(0:12.5) node[point, label=below right:{$\set{a,b,c,d,e}$}] (b-receives-add-c) {};

            \draw[dotted] (b) -- (b-initial) (b-receives-add-c) -- (b-end);
            \draw[->, thick] (b-initial) --  (b-add-d) -- (b-receives-add-c);

            \draw[->, dashed, shorten >= 1] (a-add-b) -- (b-receives-add-b);
